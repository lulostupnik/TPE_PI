    ...
    fprintf(CSVquery2, "StationA;StationB;Trips A->B;Trips B->A"); //Titulos Columnas CSV
     ...
     for (int i=0; i < size; i++) {
        ...
        for(int j=0; j<size;j++){
            ...
            nameEnd = getNameByPosition(tad, j);
...
int loadQuery3 (bikeRentingADT tad, FILE * CSVquery3, htmlTable HTquery3){
    ...
    size_t travels[12];
    char * name;
    char* januaryTrips,*februaryTrips,*marchTrips,*aprilTrips,*mayTrips,*juneTrips,*julyTrips,*augustTrips,*septemberTrips,*octoberTrips,*novemberTrips,*decemberTrips;
    
     for (int i=0; i < size; i++) {
         getTravelsByMoth(tad, i,travels);
         name = getNameByPosition(tad, i);
         januaryTrips=intToString(travels[0]);
         februaryTrips=intToString(travels[1]);
         marchTrips=intToString(travels[2]);
         aprilTrips=intToString(travels[3]);
         mayTrips=intToString(travels[4]);
         juneTrips=intToString(travels[5]);
         julyTrips=intToString(travels[6]);
         augustTrips=intToString(travels[7]);
         septemberTrips=intToString(travels[8]);
         octoberTrips=intToString(travels[9]);
         novemberTrips=intToString(travels[10]);
         decemberTrips=intToString(travels[11]);
         addHTMLRow(HTquery3,januaryTrips,februaryTrips, marchTrips,aprilTrips,mayTrips,juneTrips,julyTrips,augustTrips,septemberTrips,octoberTrips,novemberTrips,decemberTrips,name); //Voy imprimiendo el HTML
